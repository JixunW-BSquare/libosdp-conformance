...
{ /* process_osdp_input */
    ...
    if ((status EQUALS ST_PARSE_UNKNOWN_CMD) || (status EQUALS ST_BAD_CRC) || (status EQUALS ST_BAD_CHECKSUM) || (status EQUALS ST_NOT_MY_ADDR) || (status EQUALS ST_MONITOR_ONLY) || (status EQUALS ST_OK)) {
        int length;
