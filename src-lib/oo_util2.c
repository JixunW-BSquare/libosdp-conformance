...
int calc_parity(unsigned short
                    value,
    int
        length,
    int
        ...
        if (ctx->timer[i].i_sec > 0) {
            ...
            if (delta_time > 0) {
                if (ctx->timer[i].current_seconds >= delta_time)
                    ctx->timer[i].current_seconds = ctx->timer[i].current_seconds - delta_time;
                else
        ...
        if (ctx->timer[i].i_nsec > 0) {
            ...
            if (delta_nanotime > 0) {
                if (ctx->timer[i].current_nanoseconds >= delta_nanotime)
                    ctx->timer[i].current_nanoseconds = ctx->timer[i].current_nanoseconds - delta_nanotime;
                else
...
{ /* send_message */
    ...
    *current_length = 0;
    ...
    status = osdp_build_message(test_blk, // message itself
