...
{ /* osdp_create_keys */
    ...
    AES128_CBC_encrypt_buffer(ctx->s_enc, cleartext, OSDP_KEY_OCTETS, ctx->current_scbk, iv);
    if (ctx->verbosity > 8) {
        int i;
...
int osdp_setup_scbk(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* osdp_setup_scbk */
...
{ /* send_secure_message */
    ...
    *current_length = 0;
    ...
    status = osdp_build_secure_message(test_blk, // message itself
