...
int action_osdp_MFG(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* action_osdp_MFG */
...
int action_osdp_OUT(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* action_osdp_OUT */
...
int action_osdp_POLL(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* action_osdp_POLL */
...
int action_osdp_RAW(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* action_osdp_RAW */
...
{ /* action_osdp_SCRYPT */
    ...
    if (ctx->secure_channel_use[OO_SCU_ENAB] EQUALS 128 + OSDP_SEC_SCS_12) {
        ...
        if (status EQUALS ST_OK) {
            ...
            if ((0 != memcmp(server_cryptogram, ctx->rnd_b, sizeof(ctx->rnd_b))) || (0 != memcmp(server_cryptogram + sizeof(ctx->rnd_b), ctx->rnd_a, sizeof(ctx->rnd_a))))
...
int action_osdp_TEXT(OSDP_CONTEXT* ctx,
    OSDP_MSG* msg)
{ /* action_osdp_TEXT */
