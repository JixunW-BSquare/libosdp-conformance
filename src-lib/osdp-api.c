...
int process_current_command(void) { /*process_current_command */
...
void preserve_current_command(void) { /* preserve_current_command */

  char command[1024];
  char preserve[1024];

  sprintf(preserve, "%s_%02d", context.command_path, context.cmd_hist_counter);
  sprintf(command, "chmod 777 %s", context.command_path);
  system(command);

  sprintf(command, "mv %s %s", context.command_path, preserve);
  system(command);
