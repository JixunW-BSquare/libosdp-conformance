...
test_control[] = {
    ...
    { "4-8-1", &(osdp_conformance.resp_rstatr.test_status),
        0, 0, 0, 0, 0, "---" },
    { "4-9-1", &(osdp_conformance.rep_raw.test_status),
        0, 0, 0, 0, 0, "---" },
...
void skip_conformance_tests(OSDP_CONTEXT* ctx,
...
{ /* osdp_report */
    ...
    i = 0;
    while (!done) {
        if ((test_control[i].test_for_peripheral) && (*(test_control[i].conformance) != OCONFORM_SKIP)) {
