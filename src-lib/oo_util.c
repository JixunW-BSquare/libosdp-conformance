...
{ /* parse_message */
    ...
    *p;
    ...
    if (status EQUALS ST_OK) {
        ...
        if ((context->verbosity > 4) || ((m->msg_cmd != OSDP_POLL) && (m->msg_cmd != OSDP_ACK))) {
            char
                dirtag[1024];
            int i;
            unsigned char* p1;
        ...
        if ((context->verbosity > 2) || (m_dump > 0)) {
            ...
            if (((returned_hdr->command != OSDP_POLL) && (returned_hdr->command != OSDP_ACK)) || (context->verbosity > 3)) {
...
int monitor_osdp_message(OSDP_CONTEXT* context,
    OSDP_MSG* msg)
...
int process_osdp_message(OSDP_CONTEXT* context,
    OSDP_MSG* msg)
    ...
    *oh;
    ...
    if (context->role EQUALS OSDP_ROLE_PD) {
        ...
        case OSDP_BIOREAD:
            ...
            osdp_conformance.cmd_bioread.test_status = OCONFORM_EXERCISED;
            current_length = 0;
            status = send_message(context, OSDP_ACK, p_card.addr, &current_length, 0, NULL);
            context->pd_acks++;
            break;
        ...
        case OSDP_COMSET: {
            ...
            i = *(1 + msg->data_payload) + (*(2 + msg->data_payload) << 8) + (*(3 + msg->data_payload) << 16) + (*(4 + msg->data_payload) << 24);
                    ...
                    if (led_ctl->reader EQUALS 0)
                        if (led_ctl->perm_control EQUALS 1) {
                            ...
                            if (led_ctl->perm_on_color EQUALS 2)
                                osdp_conformance.cmd_led_green.test_status = OCONFORM_EXERCISED;
                        };
    ...
    if (context->role EQUALS OSDP_ROLE_CP) {
        ...
        case OSDP_LSTATR:
            ...
            fprintf(context->log,
                " Tamper %d Power %d\n",
                *(msg->data_payload + 0), *(msg->data_payload + 1));
            osdp_conformance.cmd_poll_lstatr.test_status = OCONFORM_EXERCISED; // 3-1-3
        ...
        case OSDP_PDID:
            ...
            if ((msg->data_payload[0] EQUALS 0) && (msg->data_payload[1] EQUALS 0) && (msg->data_payload[2] EQUALS 0)) {
