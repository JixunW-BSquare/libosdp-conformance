...
uint16_t CalculateChecksum_2_byte(uint8_t* data, size_t length) {
    // Initialise with this to not need the 16 bit zeroes at the end
    uint16_t crc = 0x1d0f;
...
uint8_t CalculateChecksum_1_byte(uint8_t* data, size_t length) {
    uint8_t checksum = 0;
...
int main(const int argc, const char* argv[]) {
    ...
    auto crc_1 = CalculateChecksum_1_byte(data, len);
    // printf("checksum=%d # ( 0x%02x )\n", crc_1, crc_1);

    auto crc_2 = CalculateChecksum_2_byte(data, len);
